...
public class CalculationAlgorithmTest {
    ...
    @Test
    public void shouldFailWhenBasketIsEmpty()
            throws InvalidValueOfPercentageRatio, InvalidFundsCollectionForInvestmentStyle, InvalidBasketState {
        //given
        CalculationAlgorithm calculationAlgorithm = new CalculationAlgorithm();
        java.util.List<FundEntity> emptyList = new ArrayList<>();

        //expect
        thrown.expect(InvalidBasketState.class);

        //when
        calculationAlgorithm.calculate(emptyList, mock(Money.class), mock(InvestmentStyle.class));
    }
